...
def create_loan():
    ...
    try:
        data = request.json
        success, msg = database.loans.create(data['item_id'], data['user_id'])
    except KeyError:
        success = False
...
@loans_bp.route('/read-user-loans/<id_to_read>', methods=['GET'])
@cross_origin()
@jwt_required()
def read_user_loans(id_to_read):
    user_id = get_jwt_identity()
    if not user_id:
        return jsonify({"success": False, "msg":"Operação não autorizada."}), 401
    
    user_loans = database.loans.read_user_loans(id_to_read)
